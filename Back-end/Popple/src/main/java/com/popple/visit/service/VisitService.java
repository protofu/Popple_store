...
public class VisitService {
	...
	// 시간대별 전체 통계
	public StatsResponse getTotalTime(User user, boolean isCompany) {
		Map<String, Integer> timeStats = new HashMap<>();
        // 초기값 설정
        timeStats.put("TenAM", 0);
        timeStats.put("TwelvePM", 0);
        ...
        List<Visit> visitors;
        if (isCompany) {
        	List<Exhibition> exList = exhibitionRepository.findAllByUser(user);
            visitors = visitRepository.findAllByCompanyId(user.getCompId()); // 기업 ID로 방문자 리스트 가져오기
        } else {
        	visitors = visitRepository.findAllByUserId(user.getId());
        }
        for (Visit visit : visitors) {
            int timeZone = visit.getTimeZone(); // 사용자의 시간대 가져오기
            // 시간대에 따라 통계 업데이트
